---
title: "Orders"
description: "Track domain sales in Domainer Engine from payment confirmation through transfer — order fields and status flow for manually-logged sales."
---
...
<Info>
  There is no integrated payment processing (no Stripe checkout, no escrow webhook) — see [Payment Settings](/configuration/payment-settings) for why: Buy Now is always an outbound link to an external provider. Orders are a manual bookkeeping record by design, not a byproduct of an on-site checkout — you create one yourself after a sale closes.
</Info>
...
| **Payment Link URL** | The outbound provider purchase link the buyer used, or any payment link you generated externally (Wise, PayPal, manual invoice). |
...
Create orders yourself after a sale is agreed — Buy Now, an accepted offer, or a direct negotiation off-platform:
