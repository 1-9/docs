...
| Field | Purpose |
|---|---|
| **Landing Page Layout** | Sidebar field. Assigns this domain an alternate page layout (different header style, column ratio, or price sidebar copy) instead of the site-wide Default template. Leave empty to use the Default template. See [Landing Pages](/domains/landing-pages). |
| **Hide Buy Now Button** | Sidebar field, shown when Sale Model is Buy it Now / BIN + Make Offer. Hides the Buy Now button on this domain only, overriding price/provider eligibility. See [Sale Models](/concepts/sale-models#the-hide-buy-now-button-checkbox). |
| **Hide Contact Messengers** | Sidebar field. Hides the WhatsApp/Telegram contact block on this domain only, regardless of which channels are enabled in [Messenger Settings](/configuration/messenger-settings). |
