...
| Group | Items |
|---|---|
| **Domains** | Domains, Categories, Parking, Landings |
...
| **Payments** | Buy Now toggle and text, payment providers (outbound purchase links), marketplace panel prominence — see [Payment Settings](/configuration/payment-settings) |
| **Parking** | Cloudflare account credentials, fallback redirect, TXT verification records — see [Domain Parking](/domains/parking) |
| **Messengers** | WhatsApp / Telegram contact buttons — see [Messenger Settings](/configuration/messenger-settings) |
...
Templates use a drag-and-drop block system — see [Page Builder](/content/page-builder) for details. Each template also has a [Live Preview](/content/live-preview) panel showing your unsaved edits on the real page layout.
...
- **KPIs** — total domains, available domains, page views, unique visitors, and Buy Now clicks for the selected period
- **Chart** — daily traffic for the last 7 or 30 days, including a Buy Now clicks series
- **Top Domains** — highest-traffic domain pages in the period, sortable by Buy Now clicks
- **Sources & Countries** — referrer breakdown and visitor geography. Traffic arriving through a [parked domain](/domains/parking) redirect shows as a distinct "Parking Redirect" source, separate from direct traffic
- **Recent Offers** — pending offers awaiting your response, with inline Accept / Reject / Counter actions (replaces the old Recent Orders panel, since orders are never created automatically — see [Orders](/transactions/orders))
